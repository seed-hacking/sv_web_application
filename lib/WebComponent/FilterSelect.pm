...
# $Id: FilterSelect.pm,v 1.16 2012-08-19 22:28:01 golsen Exp $
...
sub output {
  ...
  my $values_string = join("~", map {defined $_ ? $_ : ''} @{$self->values});
  $values_string =~ s/'/#/g;
  my $labels_string = join("~", map {defined $_ ? $_ : ''} @{$self->labels});
  $labels_string =~ s/'/#/g;
  my $attribute_values = [];
  my $attribute_names = [];
  my $attribute_types = [];
