...
sub output {
    ...
    my $number_of_kmers = defined($cgi->param('number_of_kmers'))? 
	                   $cgi->param('number_of_kmers') :
			     $self->number_of_kmers();
    $self->number_of_kmers($number_of_kmers);

    my $number_of_plfams = defined($cgi->param('number_of_plfams'))? 
	                   $cgi->param('number_of_plfams') :
			     $self->number_of_plfams();
    $self->number_of_plfams($number_of_plfams);

    my $number_of_pgfams = defined($cgi->param('number_of_pgfams'))? 
	                   $cgi->param('number_of_pgfams') :
			     $self->number_of_pgfams();
    $self->number_of_pgfams($number_of_pgfams);
    ...
    $number_of_sims = $number_of_pch_pins = $number_of_kmers = $number_of_plfams = $number_of_pgfams = 0;
	...
	elsif ( $select_pinned_pegs eq 'kmer' )
	{
	    $number_of_kmers     = $number_of_regions;
	}
	elsif ($select_pinned_pegs eq 'plfam')
	{
	    $number_of_plfams = $number_of_regions;
	}
	elsif ($select_pinned_pegs eq 'pgfam')
	{
	    $number_of_pgfams = $number_of_regions;
	}
	else
	{
    ...
    my $pin_desc = {
		     ...
		     'n_kmers'                => $number_of_kmers,
		     'n_plfams' 	      => $number_of_plfams,
		     'n_pgfams' 	      => $number_of_pgfams,
    ...
    {
        use Data::Dumper;
        print O Dumper($pin_desc, $fast_color, $sims_from, $region_size, $add_features, $is_annotator, $color_by_function, $peg_functions, $maps);
        close(O);
    }
...
sub pinned_regions_form {
	...
	$check1 = $self->select_pinned_pegs eq 'similarity' ? 'checked' : '';
	$check2 = $self->select_pinned_pegs eq 'kmer' ? 'checked' : '';
	$check3 = $self->select_pinned_pegs eq 'pch_pin' ? 'checked' : '';
	$check4 = $self->select_pinned_pegs eq 'plfam' ? 'checked' : '';
	$check5 = $self->select_pinned_pegs eq 'pgfam' ? 'checked' : '';
	
	$form .= "<tr><th>Pinned CDS selection</th><td>" .
	         "<input type='radio' name='select_pinned_pegs' value='similarity' $check1> Similarity " .
	         "<input type='radio' name='select_pinned_pegs' value='kmer' $check2> Kmer " .
	         "<input type='radio' name='select_pinned_pegs' value='plfam' $check4> PLfams " .
	         "<input type='radio' name='select_pinned_pegs' value='pgfam' $check5> PGfams " .
		 "<input type='radio' name='select_pinned_pegs' value='pch_pin' $check3> PCH pin</td></tr>";

	
	($check1, $check2, $check3, $check4) = ($self->collapse_close_genomes() == 1) ? ('checked', '', '', '') :
	    			      (($self->collapse_close_genomes() == 2) ? ('', 'checked', '', '') :
				       (($self->collapse_close_genomes() == 3) ? ('', '', '', 'checked') :
					('', '', 'checked', '')));
	$form .= "<tr><th>Genome selection</th><td>" .
		 ...
		 "<input type='radio' name='collapse_close_genomes' value='3' $check4> Use OTU representatives " .
    ...
    if ($self->fast_color() == 1) {
	$check1 = 'checked';
    } elsif ($self->fast_color() == 0) {
	$check2 = 'checked';
    } elsif ($self->fast_color() == 2) {
	$check3 = 'checked';
    } elsif ($self->fast_color() == 3) {
	$check4 = 'checked';
    }
...
sub pinned_regions_image {
	...
	foreach my $feature ( @$features )
	{
	    ...
	    if ($feature->{plfam})
	    {
		push @{ $line_features->[-1]{'description'} }, { title => 'PLfam',  value => $feature->{plfam} };
	    }
	    if ($feature->{pgfam})
	    {
		push @{ $line_features->[-1]{'description'} }, { title => 'PGfam',  value => $feature->{pgfam} };
	    }
...
sub pinned_regions_table {
    ...
    my $preference;
    if (defined $self->application->dbmaster) {$preference = $self->application->dbmaster->Preferences->get_objects( { user => $self->application->session->user, name => "DisplayAliasInfo" } ); }
    my $show_aliases = defined $preference && scalar(@$preference) && $preference->[0]->value() eq "show";
...
sub sequence_selection_form {
    ...
    my $downloadGBbutton  = qq(<input type="submit" class="button" name="DownloadGB" value="Download Sequences in Genbank Format">);
    my $alignbutton     = qq(<input type="submit" class="button" name="Align" value="Align Sequences">);

    $form .= $showfastabutton . "\n";
    $form .= $downloadbutton  . "\n";
    $form .= $downloadGBbutton  . "\n";
    $form .= $alignbutton     . "\n";
...
sub number_of_kmers {
  my ($self, $number_of_kmers) = @_;

  if (defined($number_of_kmers)) {
    $self->{number_of_kmers} = $number_of_kmers;
  }

  return $self->{number_of_kmers};
}

sub number_of_plfams {
  my ($self, $number_of_plfams) = @_;

  if (defined($number_of_plfams)) {
    $self->{number_of_plfams} = $number_of_plfams;
  }

  return $self->{number_of_plfams};
}

sub number_of_pgfams {
  my ($self, $number_of_pgfams) = @_;

  if (defined($number_of_pgfams)) {
    $self->{number_of_pgfams} = $number_of_pgfams;
  }

  return $self->{number_of_pgfams};
}
...
sub resolve_overlays {
  ...
  foreach my $feature (@$features) {
	...
	if ((($fs < $ie) && ($fe > $is)) || (($fe < $ie) && ($fe > $is)) || (($fs < $is) && ($fe > $ie))){
	  $conflict = 1;
	  last;
	}
      }
