...
sub output {
	...
	for (my $i=0; $i < @{$FinalList}; $i++) {
		...
		$mdlGenomes->{$FinalList->[$i]} = $mdl->genome();
    ...
    foreach my $model (@{$FinalList}) {
    	push(@{$columns},{ input => {modelgenome => $mdlGenomes->{$model},model => $model}, function => 'FIGMODELweb:display_model_gene_columns', call => 'THIS', name => $model, filter => 1, sortable => 1, width => '100', operand => $cgi->param( 'filter'.$model ) || "" });
    }
