...
function check_supercolumns (table_id) {
  ...
  if (table_supercolumns[data_index] && table_supercolumns[data_index][0][0]) {
    ...
    var col_in_table = 0;
    for (var curr_sc_num=0;curr_sc_num<sc.length;curr_sc_num++) {
      var sc_colspan = 0;
      for (var h=0;h<sc[curr_sc_num][1];h++) {
...
function table_filter (id) {
  ...
  for (z=0; z<numcols; z++) {
	...
	var operator = document.getElementById('table_' + id + '_operator_' + (z + 1)).value;
	if (operator == 'equal') {
  ...
  var newnumrows = data_array.length;
...
function table_extract_data (id, filters, nostrip) {
  ...
  for (i=0;i<table_data[data_index].length;i++) {
    ...
    var row = table_data[data_index][i];
  ...
  if (filters) {
    ...
    for (var z=0; z<filter_array.length; z++) {
...
function table_sort (id, col, dir) {
  ...
  /* determine data type */
  var sample_cell = '';
  for (var h=0;h<data_array.length;h++) {
...
function fill_table (id, start, stop, show_prev, show_next) {
  ...
  if (IE_MODE) {
    ...
    var numoldrows = table.childNodes.length - 1;
  ...
  for (var rownum=start; rownum<stop; rownum++) {
    ...
    for (var colnum=0; colnum<(cells.length - 1); colnum++) {
  ...
  var nexts = document.getElementsByName('table_next_' + id);
  var lasts = document.getElementsByName('table_last_' + id);
  var prevs = document.getElementsByName('table_prev_' + id);
  var firsts = document.getElementsByName('table_first_' + id);
...
function sort_caseinsensitive_up (a, b) {
  var aa = a[SORT_COLUMN_INDEX].toLowerCase().replace(HTML_REPLACE, "");
  var bb = b[SORT_COLUMN_INDEX].toLowerCase().replace(HTML_REPLACE, "");
  if (aa==bb) return 0;
  if (aa<bb) return -1;
  return 1;
}
...
function sort_date_up (a, b) {
  var aa = a[SORT_COLUMN_INDEX].replace(HTML_REPLACE, "");
  var bb = b[SORT_COLUMN_INDEX].replace(HTML_REPLACE, "");
  var dt1, yr, dt2;
  if (aa.length == 10) {
...
function sort_numeric_up (a, b) {
  var aa = parseFloat(a[SORT_COLUMN_INDEX].replace(HTML_REPLACE, "").replace(/,/g, ""));
  if (isNaN(aa)) return 1;
  var bb = parseFloat(b[SORT_COLUMN_INDEX].replace(HTML_REPLACE, "").replace(/,/g, ""));
...
function pivot (id, pivot_col, value_col, tdata) {
  ...
  var newnumrows = new_data.length;
  document.getElementById('table_rows_' + id).value = newnumrows;
