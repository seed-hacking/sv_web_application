...
sub output
{
    ...
    # Build summary table
    my $tabledata = [];
    #print "Filename:".$figmodel->config("database root directory")->[0]."ReactionDB/masterfiles/MapDataTable.txt";
    my $keggdata = ModelSEED::FIGMODEL::FIGMODELTable::load_table($figmodel->config("database root directory")->[0]."ReactionDB/masterfiles/MapDataTable.txt",";","|",0,undef);
	my $modelRxnTbl;
	my $modelRxnHash;
	my $modelCpdTbl;
	foreach my $model_id (@$model_ids) {
		my $modelobj = $figmodel->get_model($model_id);
		$modelCpdTbl->{$model_id} = $modelobj->compound_table();
		$modelRxnTbl->{$model_id} = $modelobj->rxnmdl();
		for (my $k=0;$k < @{$modelRxnTbl->{$model_id}}; $k++) {
			$modelRxnHash->{$model_id}->{$modelRxnTbl->{$model_id}->[$k]->REACTION()} = $modelRxnTbl->{$model_id}->[$k];
		}
	}
    for(my $i=0; $i < $keggdata->size(); $i++ ) {
		...
		my $cpd_col = "";
		my $ecs_col = "";
		for (my $j=0; $j < @$model_ids; $j++) {
			next unless (defined($modelRxnTbl->{$model_ids->[$j]}) && defined($modelCpdTbl->{$model_ids->[$j]}));
			my $rgb_string = "rgb(".$colors->[$j][0].",".$colors->[$j][1].",".$colors->[$j][2].")";
			my $model_rxn = 0;
			my $model_cpd = 0;
			my $model_ecs = 0;			
			if(defined($row->{REACTIONS})) {
				for (my $k = 0; $k < @{$row->{REACTIONS}}; $k++) {
					if(defined($modelRxnHash->{$model_ids->[$j]}->{$row->{REACTIONS}->[$k]})) {
						$model_rxn++;
					}
				}
			}
			if( defined( $row->{'COMPOUNDS'} ) ) {
				foreach( @{$row->{'COMPOUNDS'}} ) {
					if( defined( $modelCpdTbl->{$model_ids->[$j]}->get_row_by_key( $_, "DATABASE" ) ) ) {
						$model_cpd++;
					}
				}
			}
