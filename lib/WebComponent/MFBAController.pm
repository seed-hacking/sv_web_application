...
sub outputFluxControls {
	...
	if (!defined($model->userObj()) || $model->userObj()->login() eq "public") {
...
sub FBArun {
	...
	if (!defined($cgi->param('model'))) {
		return "No model specified. Cannot run FBA without model!";	
	}
	my $models = $cgi->param('model');
	$models = [split(/,/,$models)];
	my $messages = [];
	if (@{$models} > 1) {
		push(@{$message},"Multiple models specified. Only running FBA on ".$models->[0]."!");
	}
	my $model = $models->[0];
	my $media = "Complete";
	if (!defined($cgi->param('media'))) {
		push(@{$message},"No media specified. Using 'Complete' media as default!");
	} else {
		$media = $cgi->param('media');
	}
	my $mdl = $figmodel->get_model($model);
	if (!defined($mdl)) {
	...
	select STDOUT; $| = 1;
    my $fbaStartParameters = $figmodel->fba()->FBAStartParametersFromArguments({
    	arguments => {
    		model => $model,
    		media => $media
    	}
    ...
    if (!defined($results->{growth})) {
		return "FBA of model ".$model." failed. Please send note to SEED tech support!";
	}
	my $flux = "";
	my $drainFlux = "";
	if ($results->{growth} > 0.000001) {
		push(@{$message},"Growth with rate ".$results->{growth}." gm biomass/gm CDW hr.");
		...
		if (defined($results->{noGrowthCompounds}->[0])) {
			push(@{$message},"No growth. Biomass compounds ".join(",",@{$results->{noGrowthCompounds}})." could not be generated!");
		}
	}
	my $newFBAResult = {
		time => time(),
		...
		flux => $flux,
		drainFlux => $drainFlux,
		results => join("<br>",@{$message})
	};
	$figmodel->database()->create_object("fbaresult",$newFBAResult);
	open(STDOUT, ">&", $oldout);
	unlink($filename);
	return $self->loadResultsTable();
}
