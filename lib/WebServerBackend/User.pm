...
# $Id: User.pm,v 1.55 2016-02-08 17:56:46 olson Exp $
...
#
# Connect memcache at initialization time, if we have config.
#
our $memcache;
if (ref($FIG_Config::user_rights_cache))
{
    $memcache = Cache::Memcached::Fast->new($FIG_Config::user_rights_cache);
}
...
sub has_right {
  ...
  if ($application) {
      ...
      $app_id = $application->backend->_id;
  }
  

  if ($memcache)
  {
      my $key = join($;, 'has_right', $app_id, $self->_id, $right, $data_type, $data_id, $delegatable);
      my $val = $memcache->get($key);
      if (!defined($val))
      {
	  $val = $self->has_right_impl($application, $app_id, $right, $data_type, $data_id, $delegatable);
	  $val = 0 if !defined($val);
	  $memcache->set($key, $val, $FIG_Config::user_rights_expiration_time);
	  # print STDERR "Stored value $val for key $key\n";
      }
#       else
#       {
# 	  print STDERR "Using cached value $val for key $key\n";
#       }
  ...
  {
      return $self->has_right_impl($application, $app_id, $right, $data_type, $data_id, $delegatable);
  }
}

sub has_right_impl {
  my ($self, $application, $app_id, $right, $data_type, $data_id, $delegatable) = @_;
...
sub flush_cache_has_right_to
{
    ...
    if ($application) {
	unless (ref $application && $application->isa('WebApplication')) {
	    die "Method User->has_right_to called without a valid application parameter.\n";
	}
	$app_id = $application->backend->_id;
    }
    ...
    if ($memcache)
    {
	my $key = join($;, 'has_right_to', $app_id, $self->_id, $right, $data_type);
	$memcache->delete($key);
    }
}
...
sub has_right_to {
  ...
  #
  # Hoist this from the impl so we can cache.
  #
  my $app_id;
  if ($application) {
      unless (ref $application && $application->isa('WebApplication')) {
  ...
  if ($memcache)
  {
      ...
      if (!defined($val))
      {
	  $val = $self->has_right_to_impl($application, $app_id, $right, $data_type);
	  $memcache->set($key, $val, $FIG_Config::user_rights_expiration_time);
	  # print STDERR "Stored value for key $key: " . Dumper($val);
      }
#       else
#       {
# 	  print STDERR "Using cached value for key $key: " . Dumper($val);
#       }
  ...
  {
      return $self->has_right_to_impl($application, $app_id, $right, $data_type);
  }
}

sub has_right_to_impl {
  my ($self, $application, $app_id, $right, $data_type) = @_;
...
sub wants_rast_jobs_starting_with
{
    ...
    #
    # If we are on rast-test, return all jobs.
    #
    if (grep { /rast-test/ } @INC)
    {
	return 1;
    }
    elsif (scalar(@$pref) && $pref->[0]->value > 0)
    {
	return $pref->[0]->value;
    }
    else
