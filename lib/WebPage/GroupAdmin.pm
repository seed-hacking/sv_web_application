...
sub output {
	...
	if ($application->session->user->has_right($application, 'edit', 'scope', $scope_object->_id())) {
		...
		$admin = $self->start_form( 'group_form', { 'action' => 'make_group_admin',
							    ...
							    'scope' => $scope_object->_id() } 
		    )
		    ."<input type='submit' value='make admin' style='height: 20px;'>" 
		    .$self->end_form();
	    }
		...
		."</td><td><input type='submit' value='remove' style='height: 20px;'>";
    ...
    $users_field .= "<hr><b>Add user by eMail</b><br>"
	.$self->start_form( 'scope_form', { 'action' => 'grant_scope', 'group' => $scope } );
	...
	. $self->start_form('group_name_form', { 'action' => 'change_group_name', 'group' => $scope }) 
	. "<input type='text' value='$scope' name='new_group' style='width: 220px;'><input type='submit' value='change'>" 
	.$self->end_form() 
	. "</td></tr>";
    $html .= "<tr><th>Description</th><td>" 
	. $self->start_form('group_description_form', { 'action' => 'change_group_description', 'group' => $scope }) 
	. "<textarea name='group_description' cols='25' >" 
	. $scope_description 
	. "</textarea><input type='submit' value='change'>" 
	.$self->end_form() 
	. "</td></tr>";
...
sub revoke_scope {
    ...
    $application->add_message('info', join(q( ), ('User', $user->firstname(),  $user->lastname(),
						  'removed from group', $scope->name())
			      )
	);
}
