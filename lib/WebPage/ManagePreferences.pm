...
sub editable_preferences {
  ...
  if ($self->application->backend->name eq 'MGRAST' || $self->application->backend->name eq 'RNASEQRAST') {
    return {
	    ...
	    "WebServicesKey" => { value => "",
				  ...
				  info => "<b>Note:</b> Creating a new key<br>will render the previous key deprecated. Your key will be valid for a limited time only (see webkey termination date). You can generate a new key with a new termination date at any time." },
  ...
  return { "ComparedRegionsDefaultNumRegions" => { value => 15,
	   ...
	   "WebServicesKey" => { value => "",
				 ...
				 generator => "web_services_key_generator",
				 category => "Web Services",
				 info => "<b>Note:</b> Creating a new key <br>will render the previous key deprecated.<br>For more information on WebServices click <a href='http://ws.nmpdr.org/' target=_blank>here</a>" },
